...
<script type="text/javascript">
	$(document).ready(function(){
		...
		<tr>
			...
			<td>:</td>
			<td colspan="2">
			<select id="<%=Keys.REPORT_FOR%>" name="<%=Keys.REPORT_FOR%>" onchange="dateOption(this)">
			<option value="<%=Keys.REPORT_FOR_LAST_7_DAYS%>">Last 7 Days</option>
			<option value="<%=Keys.REPORT_FOR_LAST_15_DAYS%>">Last 15 Days</option>
			<option value="<%=Keys.REPORT_FOR_LAST_30_DAYS%>">Last 30 Days</option>
			<option value="<%=Keys.REPORT_FOR_LAST_60_DAYS%>">Last 60 Days</option>
			<option value="<%=Keys.REPORT_FOR_LAST_90_DAYS%>">Last 90 Days</option>
		...
		<tr id="<%=Keys.REPORT_DATE_TR%>">
			...
			<td>:</td>
			<td align="left" colspan="2"><input type="text" id="<%=Keys.REPORT_DATE_FROM%>" name="<%=Keys.REPORT_DATE_FROM%>" size="8" readonly value=""  style="width:100px;"> to <input type="text" id="<%=Keys.REPORT_DATE_TO%>" name="<%=Keys.REPORT_DATE_TO%>" size="8" readonly value=""  style="width:100px;"></td>
		</tr>
		
		

		<tr> <td colspan="4" align="right"> <div class="jbutton"><input type="submit" name="SUBMIT" value="Get" style="width:80px;"></div></td></tr>
		</table>
...
<%
		...
		ArrayList<TestSuite> testSuites = new ReportTrend().getTrendReport((String)request.getParameter(Keys.REPORT_FOR), Integer.parseInt((String)request.getParameter("references")), (String)request.getParameter(Keys.REPORT_DATE_FROM), (String)request.getParameter(Keys.REPORT_DATE_TO));
		...
		if(testSuites.size()>0){
			testRef = testSuites.get(testSuites.size()-1).getTestReference();
			suiteName = testSuites.get(testSuites.size()-1).getTestSuiteName();
		}
		
		for(TestSuite tmpSuite : testSuites){
			timeStamp = tmpSuite.getLocalStartTime().getTime();
			if(passedCases != null){
				passedCases 	+= ",["+timeStamp+","+tmpSuite.getPassedCases()+"]";
				failedCases 	+= ",["+timeStamp+","+tmpSuite.getFailedCases()+"]";
				skippedCases	+= ",["+timeStamp+","+tmpSuite.getSkippedCases()+"]";
			}else{
				passedCases 	= "["+timeStamp+","+tmpSuite.getPassedCases()+"]";
				failedCases 	= "["+timeStamp+","+tmpSuite.getFailedCases()+"]";
				skippedCases	= "["+timeStamp+","+tmpSuite.getSkippedCases()+"]";
		...
		<div id="dt_page">
		<div id="container">
		<h1>Trend Report: <font size="2">[<b>Test Suite: </b><I><%=suiteName%>,</I> <b>Reference:</b> <I><%=testRef%>, </I><b>Date Range:</b> <I><%=reportFor+dateRange%></I>]</font></h1>

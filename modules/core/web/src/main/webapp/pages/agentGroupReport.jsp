...
<%
	if(buttonName.equalsIgnoreCase("Delete")){
		...
		response.sendRedirect("agentGroupReport.jsp");
...
%>		
		...
		<form method="post" name="jobDetails" action="agentGroupReport.jsp"> 
		<table border="1" cellpadding="3" align="left" id="dt_table">
		...
		<tr>
			...
			<td colspan="2"><input type="checkbox" name="<%=Keys.REPORT_EMAIL_GROUP_GROUP_ENABLED%>" id="<%=Keys.REPORT_EMAIL_GROUP_GROUP_ENABLED%>" value="groupEnabled" 
			<%if(reportGroup.isTestSuiteGroupEnabled()){%> checked <%}%> ></td>
			<td><input type="text" name="<%=Keys.REPORT_EMAIL_GROUP_NAME%>" size="2" value="<%=reportGroup.getGroupName()%>"/></td>
			<td><select>
...
%>		
		...
		<tr>
			<td align="left">Group Name</td>
			<td style="width: 2px; overflow-x: hidden;">:</td>
			<td colspan="3"><input type="text" name="<%=Keys.REPORT_EMAIL_GROUP_NAME%>" size="8" value=""  style="width:320px;"></td>
		</tr>
		...
		<tr>
			<td align="left">Test Reference(s)</td>
			<td>:</td>
			<td colspan="3"><select id="<%=Keys.REPORT_EMAIL_GROUP_REFERENCE%>" name="<%=Keys.REPORT_EMAIL_GROUP_REFERENCE%>" multiple="multiple">
			<%
				ArrayList<TestReference> testReferences = new ManageTestReference().getAllTestReference();
				for(TestReference testReference : testReferences){
		...
		<tr>
			<td align="left">Test Suite Group (Include)</td>
			<td valign="top">:</td>
			<td colspan="3"><input type="checkbox" name="<%=Keys.REPORT_EMAIL_GROUP_GROUP_ENABLED%>" id="<%=Keys.REPORT_EMAIL_GROUP_GROUP_ENABLED%>" value="groupEnabled" ></td>	
		</tr>
		...
		<tr>
			<td align="left">CPU</td>
			<td valign="top">:</td>
			<td><input type="checkbox" name="<%=Keys.REPORT_EMAIL_GROUP_GROUP_ENABLED%>" id="<%=Keys.REPORT_EMAIL_GROUP_GROUP_ENABLED%>" value="groupEnabled" checked /></td>
			<td><input type="text" name="<%=Keys.REPORT_EMAIL_GROUP_NAME%>" size="2" value="5"/></td>
			<td><select>
    				<option value="seconds"  selected="selected">Seconds</option>
    				<option value="minutes">Minutes</option>
    				<option value="hours">Hours</option>
		...
		<tr> <td colspan="5" align="right"> <div class="jbutton"><input type="submit" name="SUBMIT" value="Submit" style="width:80px;"></div></td></tr>
		</table>
...
<h1>Groups: Agent(s)</h1>
<form method="post" id="manage-reports" name="manageReports" action="agentGroupReport.jsp" >

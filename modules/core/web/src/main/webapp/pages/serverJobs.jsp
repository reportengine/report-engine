...
Server server = new ServerTable().getById(Integer.parseInt(serverId));
...
<%
	if(buttonName.equalsIgnoreCase("Delete")){
		...
		response.sendRedirect("serverJobs.jsp?"+Keys.SERVER_ID+"="+serverId);
...
<script type="text/javascript">
	$(document).ready(function(){
      ...
      $('.chosen1').chosen({ width: "100%" });
		...
		<div id="dt_page">
		<div id="container">
		<h1>Add New Schedule: <font size="2">[<b>Server: </b><I><%=server.getName()%>,</I>
				<b>Host/IP:</b> <I><%=server.getHostIp()%>,</I> <b>Platform:</b> <I><%=server.getPlatform()%></I>]
			</font>
	</h1>

		<form method="post" name="jobDetails" action="serverJobs.jsp"> 
	...
	<tr id=<%=Keys.JOB_DATE_TO_TR%>>
		...
		<input type="hidden" id="<%=Keys.TYPE%>" name="<%=Keys.TYPE%>" value="<%=JobClasses.TYPE.AGENT%>">		
...
<h1>Schedules: <font size="2">[<b>Server: </b><I><%=server.getName()%>,</I>
				<b>Host/IP:</b> <I><%=server.getHostIp()%>,</I> <b>Platform:</b> <I><%=server.getPlatform()%></I>]
			</font>
	</h1>

<form method="post" id="manage-reports" name="manageReports" action="serverJobs.jsp" >
<table cellpadding="0" cellspacing="0" border="0" class="display" id="dt_table">
	...
	<%
		ArrayList<JobScheduler> jobs=  new JobSchedulerTable().getJobsByServerId(Integer.valueOf(serverId));		
...
<BR>
<div class="jbutton">
	<input type="submit" name="SUBMIT" value="Add" style="width:80px;"> <input type="submit" name="SUBMIT" value="Delete" style="width:80px;"> <input type="submit" name="SUBMIT" value="Enable" style="width:80px;"> <input type="submit" name="SUBMIT" value="Disable" style="width:80px;">
</div>	

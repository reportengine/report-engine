...
<script type="text/javascript">
	$(document).ready(function(){
      ...
      $('.chosen2').chosen({ width: "100%" });
		...
		$('#<%=Keys.SERVER_ID%>').change(function() {
	        var selectedValue = $(this).val();
	        var servletUrl = 'ajaxJvm.jsp?<%=Keys.SERVER_ID%>='+selectedValue;

	        $.getJSON(servletUrl, function(options) {
	            var dropdown2 = $('#<%=Keys.JVM_NAME%>');
...
<div id="dt_page">
	<div id="container">
		<h1>JVM Memory Report:</h1>

		<form method="post" name="jvmChart" action="chartJvmMemory.jsp">
			<table border="0" cellpadding="3" align="left" id="dt_table">

				<tr>
					<td align="left">Server</td>
					<td>:</td>
					<td colspan="2"><select data-placeholder="Choose a Server..."  tabindex="1" class="chosen1" id="<%=Keys.SERVER_ID%>" name="<%=Keys.SERVER_ID%>">            <option value=""></option> 
							<%
				...
				<tr>
					...
					<td>:</td>
					<td colspan="2"><select
						data-placeholder="Choose a Server..." tabindex="2"
						class="chosen2" name="<%=Keys.JVM_NAME%>"
						id="<%=Keys.JVM_NAME%>">
							<%
				...
				<tr>
					...
					<td>:</td>
					<td colspan="2"><input type="checkbox" name="<%=Keys.CHART_JVM_MEMORY_HEAP%>" id="<%=Keys.CHART_JVM_MEMORY_HEAP%>" value="Enabled" checked disabled="disabled"></td>
				</tr>
				
				<tr>
					<td align="left">Non Heap Used</td>
					<td>:</td>
					<td colspan="2"><input type="checkbox" name="<%=Keys.CHART_JVM_MEMORY_NON_HEAP%>" id="<%=Keys.CHART_JVM_MEMORY_NON_HEAP%>" value="Enabled"></td>
				</tr>
...
$(function() {
  $( "#test-suite-resource-usage-tabs" ).tabs();
		...
		<div id="test-suite-resource-usage-tabs" style="overflow:hidden;">
			...
			tabHeader.append("\n</ul>");
  			...
  			String type = request.getParameter(Keys.SUBMIT);
				...
				//data.setSelectedResources("'"+Keys.CHART_JVM_MEMORY_HEAP+"','"+Keys.CHART_JVM_MEMORY_NON_HEAP+"'");
				data.setSelectedResources("'"+Keys.CHART_JVM_MEMORY_HEAP+"'");
  			...
  			for(TestReferenceServerMap serverMap : serverMaps){
  				...
  				<%
  					if(new Charts().isDataAvailable(data, RESOURCE_TYPE.JVM_MEMORY)){
  				...
  				function createChart() {
					$('#chartContainer<%=data.getServerId()%>').highcharts('StockChart', {
	  				    ...
	  				    tooltip: {
  					    	...
  					    	pointFormat: '<span style="color:{series.color}">{series.name}</span>: {point.y}MB<br/>',
  					    	valueDecimals: 2
  					    },
	...
	<%
			...
			DynamicTableName tableName = DynamicTableNameTable.getById(Integer.valueOf(jvmId));
			
			
			StringBuilder selectedResources = new StringBuilder();
			selectedResources.append("'").append(Keys.CHART_JVM_MEMORY_HEAP).append("'");
			
			if(General.isCheckBoxEnabled((String)request.getParameter(Keys.CHART_JVM_MEMORY_NON_HEAP))){
				selectedResources.append(",'").append(Keys.CHART_JVM_MEMORY_NON_HEAP).append("'");
			}
...
$(function() {
	...
	var err ;
	$.each(names, function(i, name) {
		$.getJSON($(location).attr('href').replace('chartJvmMemory.jsp','')+'ajaxChart.jsp?'+'<%=Keys.SERVER_ID%>=<%=serverId%>&<%=Keys.JVM_NAME%>=<%=jvmId%>&<%=Keys.REPORT_FOR%>=<%=reportFor%>&<%=Keys.REPORT_DATE_FROM%>=<%=URLEncoder.encode(fromStrDate, Keys.URL_ENCODE_UTF_8)%>&<%=Keys.REPORT_DATE_TO%>=<%=URLEncoder.encode(toStrDate, Keys.URL_ENCODE_UTF_8)%>&<%=Keys.RESOURCE_TYPE%>='+ name).done(function(data) {
...
<div id="dt_page">
	<div id="container">
		<h1>
			JVM Memory Report: <font size="2">[<b>Server: </b><I><%=server.getName()%>,</I>
				<b>Host/IP:</b> <I><%=server.getHostIp()%>,</I> <b>Platform:</b> <I><%=server.getPlatform()%>,</I> <b>Time Range:</b> <I><%=reportFor%></I> <b>JVM:</b> <I><%=tableName.getName().replace("_"+server.getId(), "")%></I>]
			</font>
		</h1>
